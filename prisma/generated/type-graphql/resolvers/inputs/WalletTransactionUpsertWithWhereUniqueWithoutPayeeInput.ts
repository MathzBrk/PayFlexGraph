import * as TypeGraphQL from "type-graphql";
import * as GraphQLScalars from "graphql-scalars";
import { Prisma } from "@prisma/client";
import { DecimalJSScalar } from "../../scalars";
import { WalletTransactionCreateWithoutPayeeInput } from "../inputs/WalletTransactionCreateWithoutPayeeInput";
import { WalletTransactionUpdateWithoutPayeeInput } from "../inputs/WalletTransactionUpdateWithoutPayeeInput";
import { WalletTransactionWhereUniqueInput } from "../inputs/WalletTransactionWhereUniqueInput";

@TypeGraphQL.InputType("WalletTransactionUpsertWithWhereUniqueWithoutPayeeInput", {})
export class WalletTransactionUpsertWithWhereUniqueWithoutPayeeInput {
  @TypeGraphQL.Field(_type => WalletTransactionWhereUniqueInput, {
    nullable: false
  })
  where!: WalletTransactionWhereUniqueInput;

  @TypeGraphQL.Field(_type => WalletTransactionUpdateWithoutPayeeInput, {
    nullable: false
  })
  update!: WalletTransactionUpdateWithoutPayeeInput;

  @TypeGraphQL.Field(_type => WalletTransactionCreateWithoutPayeeInput, {
    nullable: false
  })
  create!: WalletTransactionCreateWithoutPayeeInput;
}
